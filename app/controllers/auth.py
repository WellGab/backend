...
class AuthController:
    ...
    def sign_up(self, user_data: auth_schema.SignUpSchema) -> auth_schema.SignUpResponse: 
        ...
        new_user = auth_service.AuthService().create_user(email=user_data.email, password=user_data.password)
    ...
    def login_in(self, user_data: auth_schema.LoginSchema) -> auth_schema.LoginResponse:
        """Verifies login credentials and returns access token."""
        user: User = user_service.UserService().get_user(user_data.email)

        # Create user document
        id = str(user.id)
        user = auth_service.AuthService().login_user(id=id, password=user_data.password, hashed_password=user.password)
