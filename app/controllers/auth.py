...
class AuthController:
    ...
    def sign_up(user_data: auth_schema.SignUpSchema) -> auth_schema.AuthResponse:
        ...
        new_user = auth_service.AuthService.create_user(
            email=user_data.email, password=user_data.password, auth_channel=AUTH_CHANNEL_DEFAULT)
    ...
    def social_auth(user_data: auth_schema.SocialAuthSchema) -> auth_schema.AuthResponse:
        ...
        # Login user
        id = str(user.id)
        user = auth_service.AuthService.login_user(id=id, password=password, hashed_password=user.password)
